...
export const AuthForm = ({ type }: { type: 'signin' | 'signup' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const user = res.user ?? (res.userId ? { id: res.userId, username: fallbackUsername, email } : null);
      setAuth(user, token);
  ...
  return (
    <div className="app-shell ambient-noise flex min-h-screen flex-col lg:flex-row">
      ...
      <div className="flex flex-1 items-center justify-center p-5 sm:p-8">
        <div className="glass animate-fade-in-up w-full max-w-[480px] rounded-[24px] p-6 sm:p-9"
          style={{ boxShadow: '0 24px 56px rgba(43,35,27,.18), 0 1px 3px rgba(43,35,27,.08)' }}>
          ...
          <div className="mb-5 flex items-center justify-between gap-2.5">
            <div className="flex items-center gap-2.5">
              <div className="grid h-10 w-10 place-items-center rounded-[12px] border border-[rgba(26,26,26,.18)] bg-[rgba(13,91,215,.08)] p-1.5">
                <Image src="/logo.svg" alt="Pencil.io logo" width={28} height={28} priority />
              </div>
            <span className="text-[1rem] font-bold tracking-[-0.02em]">Pencil.io</span>
            </div>
          ...
          <h2 className="ink-title mb-6 mt-2 text-[1.8rem] sm:text-[2.1rem]">
            {type === 'signin' ? 'Welcome back to your studio' : 'Open your collaborative studio'}
          </h2>
          ...
          <form onSubmit={handleSubmit} className="grid gap-3">
            ...
            {type === 'signup' && <p className="-mt-1 text-[0.74rem] soft-copy">Strong password required.</p>}
            ...
            <button className="btn btn-primary btn-lg mt-1 flex w-full items-center justify-center gap-2" disabled={loading}>
              {loading && (
      ...
      >
        ...
        <div className="absolute left-9 top-10 z-20 max-w-[480px]">
          ...
          <p className="mt-3 max-w-[420px] text-[0.96rem] leading-[1.55] soft-copy">
            Pencil.io blends canvas, chat, and media into one cinematic workspace where ideas stay in motion.
          </p>
        </div>
        ...
        <div
          ...
          style={{ backdropFilter: 'blur(6px)', animation: 'floatOrb 12s ease-in-out infinite' }}
        >
          "From first sketch to final decision, every collaborator stays in the same rhythm."
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 z-20 border-t border-[rgba(26,26,26,.1)] bg-[rgba(255,250,241,.72)] px-9 py-5">
          <div className="flex items-center justify-between gap-6">
            <div>
              <p className="text-[0.72rem] uppercase tracking-[0.11em] text-[var(--ink-soft)]">Experience</p>
              <p className="mt-1 text-[1.05rem] font-semibold text-[var(--ink)]">Crafted for high-end product experiences</p>
            </div>
            <div className="grid grid-cols-3 gap-2.5 text-center">
              <div className="rounded-xl border border-[rgba(26,26,26,.15)] bg-[rgba(255,250,241,.86)] px-3 py-2">
                <p className="text-[1rem] font-semibold text-[var(--ink)]">12ms</p>
                <p className="text-[0.66rem] uppercase tracking-[0.08em] text-[var(--ink-soft)]">Sync</p>
              </div>
              <div className="rounded-xl border border-[rgba(26,26,26,.15)] bg-[rgba(255,250,241,.86)] px-3 py-2">
                <p className="text-[1rem] font-semibold text-[var(--ink)]">3-in-1</p>
                <p className="text-[0.66rem] uppercase tracking-[0.08em] text-[var(--ink-soft)]">Workspace</p>
              </div>
              <div className="rounded-xl border border-[rgba(26,26,26,.15)] bg-[rgba(255,250,241,.86)] px-3 py-2">
                <p className="text-[1rem] font-semibold text-[var(--ink)]">Live</p>
                <p className="text-[0.66rem] uppercase tracking-[0.08em] text-[var(--ink-soft)]">Presence</p>
              </div>
            </div>
          </div>
        </div>
