...
import { getHealthStatus, getInternalStatus, runReplayCheck } from "./internal/status";
import { URL } from "url";
import fs from "fs";
...
import { normalizeHeaderValue, resolveCorsOrigin, withCorsHeaders } from "./utils/http.util";

const requestCounters = new Map<string, { count: number; resetAt: number }>()
const MAX_REQUEST_COUNTER_KEYS = Number(process.env.MAX_REQUEST_COUNTER_KEYS || 50_000)
...
function sendJson(req: any, res: any, statusCode: number, body: unknown) {
  res.writeHead(statusCode, withCorsHeaders(req, { "content-type": "application/json" }))
...
function sanitizeUploadPath(urlPathname: string): { absolutePath: string; relativePath: string; mimeType: string } | null {
...
async function readRawBody(req: any, limitBytes = 16 * 1024 * 1024): Promise<Buffer> {
  const chunks: Buffer[] = []
  let total = 0
...
async function fetchUploadFromPeers(relativePath: string): Promise<Buffer | null> {
...
export async function startSocketServer() {
    ...
    if (parsedUrl.pathname.startsWith("/upload/")) {
      ...
      if (req.method === "PUT") {
        ...
        if (!uploadSignature || !uploadContentType || !Number.isFinite(expiresAt)) {
          sendJson(req, res, 400, { message: "Missing upload signature headers" })
          return
        }
        ...
        if (!validUploadTicket) {
          sendJson(req, res, 403, { message: "Invalid or expired upload signature" })
          return
        }
        ...
        try {
          ...
          fs.writeFileSync(uploadPath.absolutePath, body)
      ...
      if (req.method === "GET") {
        if (!fs.existsSync(uploadPath.absolutePath)) {
          const remoteData = await fetchUploadFromPeers(uploadPath.relativePath)
          if (!remoteData) {
          ...
          const dir = path.dirname(uploadPath.absolutePath)
          fs.mkdirSync(dir, { recursive: true })
          fs.writeFileSync(uploadPath.absolutePath, remoteData)
        }
  ...
  server.on("upgrade", (request, socket, head) => {
    ...
    const parsed = request.url ? new URL(request.url, "http://localhost") : null
