...
export const HeroSection = () => {
  ...
  const joinRoom = async () => {
    ...
    window.location.href = activeToken
      ? `/room/${normalized}`
      : `/auth/signin?join=${encodeURIComponent(normalized)}`;
  };
    ...
    <section className="app-shell ambient-noise relative grid min-h-[calc(100vh-90px)] place-items-center px-4 pb-16 pt-10 sm:px-6 sm:pt-14">
      <div className="pointer-events-none absolute inset-0">
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 35% 35%, rgba(13,91,215,.5), rgba(13,91,215,.06))",
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 35% 35%, rgba(227,106,31,.46), rgba(227,106,31,.08))",
      ...
      <div className="relative z-[1] mx-auto grid w-full max-w-[1180px] items-start gap-6 lg:items-end lg:grid-cols-[1.2fr_.8fr]">
        <div className="animate-fade-in-up">
          ...
          <h1 className="ink-title m-0 max-w-[780px] text-[clamp(2.4rem,8vw,6.4rem)]">
            ...
            <br />
            <span className="line-scribble bg-[linear-gradient(95deg,#1a1a1a,#0d5bd7_52%,#e36a1f_95%)] bg-clip-text text-transparent">
              a live studio.
            </span>
          </h1>
          ...
          <p className="soft-copy mt-3.5 text-[0.82rem]">
            Used by product squads, growth teams, and design studios shipping
            weekly.
          </p>
        </div>
        ...
        <div
          id="why"
          className="glass animate-fade-in-up rounded-[20px] border-[rgba(26,26,26,.17)] p-5 [animation-delay:110ms]"
        >
          <p className="mb-2 text-[0.74rem] font-semibold uppercase tracking-[0.14em] text-[var(--ink-soft)]">
            Why teams stay
          </p>
          <ul className="m-0 grid list-none gap-2 p-0 text-[0.95rem]">
            <li className="rounded-xl border border-[rgba(26,26,26,.14)] bg-[rgba(255,250,241,.65)] px-3 py-2">
              Canvas + chat in one room id
            </li>
            <li className="rounded-xl border border-[rgba(26,26,26,.14)] bg-[rgba(255,250,241,.65)] px-3 py-2">
              Live connection status and graceful reconnect
            </li>
            <li className="rounded-xl border border-[rgba(26,26,26,.14)] bg-[rgba(255,250,241,.65)] px-3 py-2">
              Fast collaboration patterns without tool bloat
            </li>
          </ul>
          <div className="mt-4 rounded-xl border border-dashed border-[rgba(26,26,26,.22)] p-3 text-[0.85rem] soft-copy">
            Human-centered by design: less chrome, more clarity, better
            momentum.
          </div>
        </div>
      </div>
  ...
  <section id="features" className="px-4 pb-24 pt-4 sm:px-6">
    <div className="mx-auto grid w-full max-w-[1200px] gap-3 md:grid-cols-3">
      <div className="glass rounded-2xl p-5">
        ...
        <p className="m-0 text-sm leading-6 soft-copy">
          Grid-aware drawing with lightweight tools, smooth pan and object-level
          updates.
        </p>
      </div>
      <div className="glass rounded-2xl p-5">
        ...
        <p className="m-0 text-sm leading-6 soft-copy">
          Presence, grouped chat, and instant visual acknowledgement on shared
          edits.
        </p>
      </div>
      <div className="glass rounded-2xl p-5">
        ...
        <p className="m-0 text-sm leading-6 soft-copy">
          Sync states, reconnect awareness, and busy indicators designed for
          real traffic.
        </p>
      </div>
    </div>
