...
export const handleChatSend = async function (
  ...
  if (!content || typeof content !== "string") return sendSocketError(socket, "content is required");
  if (content.length > 10000) return sendSocketError(socket, "message too long");
  ...
  try {
    ...
    if (!membership) return sendSocketError(socket, "Not a member of this room");
