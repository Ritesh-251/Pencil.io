...
export function JoinRequestPopup({ roomId, requests, onHandled }: JoinRequestPopupProps) {
  const latest = requests[0];
  if (!latest) return null;

  const handleApprove = async () => {
    try {
      await api.post(`/api/v1/rooms/${roomId}/join-requests/${latest.id}/approve`);
      onHandled(latest.id);
    } catch (err) {
  ...
  const handleReject = async () => {
    try {
      await api.post(`/api/v1/rooms/${roomId}/join-requests/${latest.id}/reject`);
      onHandled(latest.id);
    } catch (err) {
      ...
      <div className="glass flex flex-col gap-3 rounded-2xl border border-[rgba(13,91,215,0.2)] bg-[rgba(255,250,241,0.95)] p-4 shadow-2xl w-80">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[0.88rem] font-bold text-[#1a1a1a] truncate">New Join Request</p>
            <p className="text-[0.78rem] text-[#666] truncate">{latest.userEmail}</p>
          </div>
        </div>
