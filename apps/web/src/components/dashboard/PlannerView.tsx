    ...
    <div className="flex flex-col gap-6 h-full">
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="soft-copy mt-1">Manage your upcoming collaborative sessions.</p>
        </div>
      ...
      {meetings.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center p-12 border border-dashed border-[rgba(26,26,26,0.1)] rounded-[32px]">
          ...
          <p className="soft-copy text-center">No meetings scheduled yet.<br/>Create one to invite your team.</p>
        </div>
      ) : (
        <div className="grid gap-4 overflow-y-auto pr-2 custom-scrollbar">
          {meetings.map((meeting) => (
            <div key={meeting.id} className="glass border border-[rgba(26,26,26,0.06)] p-5 rounded-[24px] hover:border-indigo-500/30 transition-all group">
              ...
              <div className="mt-4 pt-4 border-t border-[rgba(26,26,26,0.05)] flex justify-between items-center">
                ...
                <button 
                  ...
                  onClick={() => window.location.href = `/room/${meeting.roomId}`}
                >
                  Join Room →
                </button>
              </div>
