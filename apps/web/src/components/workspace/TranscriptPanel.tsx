...
function toSpeakerName(participant: { name?: string | null; identity?: string }) {
...
export function TranscriptPanel() {
  ...
  const panelStatus = useMemo(() => {
    if (!transcriptEnabled) {
      return { label: 'Transcript off', className: 'status-pill status-pill-red' };
    }
    if (!room || status === 'disconnected' || status === 'failed') {
      return { label: 'Agent disconnected', className: 'status-pill status-pill-red' };
    }
    if (isTranscribing) {
      return { label: 'Transcribing…', className: 'status-pill status-pill-blue' };
    }
    return { label: 'Agent connected', className: 'status-pill status-pill-green' };
      ...
      <div ref={scrollRef} className="scroll-thin min-h-0 flex-1 overflow-y-auto px-3 py-3">
        {!transcriptEnabled ? (
          ...
          <div className="flex flex-col gap-2">
            {entries.map((entry) => {
              const speaking = activeSpeakers.includes(entry.participantIdentity);
              return (
                <div key={entry.id} className="rounded-xl border border-[rgba(26,26,26,.12)] bg-[rgba(255,250,241,.86)] px-3 py-2">
                  <div className="mb-1 flex items-center gap-1.5 text-[0.7rem] text-[var(--ink-soft)]">
                    <span>[{formatClock(entry.ts)}]</span>
                    <span>·</span>
                    <span className="font-semibold text-[var(--brand-strong)]">{entry.speaker}</span>
                    <span className="ml-auto flex items-center gap-1">
                      <IconMic active={speaking} />
                    </span>
                  </div>
                  <div className="text-[0.86rem] leading-5 text-[var(--ink)]">{entry.text}</div>
                </div>
              );
            })}
          </div>
