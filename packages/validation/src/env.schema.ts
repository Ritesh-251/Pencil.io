...
export const HttpBackendEnvSchema = CommonEnvSchema.extend({
  PORT: z.string().transform(Number).default(3001 as any),
...
export const WsBackendEnvSchema = CommonEnvSchema.extend({
  PORT: z.string().transform(Number).default(3003 as any),
  RABBITMQ_URL: z.string().url(),
...
export const AiServiceEnvSchema = CommonEnvSchema.extend({
  PORT: z.string().transform(Number).default(3004 as any),
  RABBITMQ_URL: z.string().url(),
  GEMINI_API_KEY: z.string(),
  GEMINI_CHAT_MODEL: z.string().default("gemini-1.5-flash"),
  GEMINI_SUMMARY_MODEL: z.string().default("gemini-1.5-flash"),
  GEMINI_EMBED_MODEL: z.string().default("text-embedding-004"),
  GEMINI_EMBED_DIMENSION: z.string().transform(Number).default(1536 as any),
  OLLAMA_BASE_URL: z.string().url().default("http://localhost:11434"),
  OLLAMA_MODEL: z.string().default("qwen3.5:2b"),
  INTERNAL_SECRET: z.string().default("YOUR_SHARED_SECRET"),
});
