...
const RAISE_HAND_TIMEOUT_MS  = 60_000;
...
function ParticipantTile({
  participant, identity, isLocal, expanded,
  isSpeaking, isHandRaised, isPinned, reactions = [], onPinToggle,
}: TileProps) {
  ...
  useEffect(() => {
    ...
    participant
      .on(ParticipantEvent.TrackPublished, refresh).on(ParticipantEvent.TrackSubscribed, refresh)
      .on(ParticipantEvent.TrackUnpublished, refresh).on(ParticipantEvent.TrackUnsubscribed, refresh)
      .on(ParticipantEvent.TrackMuted, refresh).on(ParticipantEvent.TrackUnmuted, refresh)
      .on(ParticipantEvent.LocalTrackPublished, refresh).on(ParticipantEvent.LocalTrackUnpublished, refresh)
      .on(ParticipantEvent.ParticipantNameChanged, refresh);
    return () => {
      participant
        .off(ParticipantEvent.TrackPublished, refresh).off(ParticipantEvent.TrackSubscribed, refresh)
        .off(ParticipantEvent.TrackUnpublished, refresh).off(ParticipantEvent.TrackUnsubscribed, refresh)
        .off(ParticipantEvent.TrackMuted, refresh).off(ParticipantEvent.TrackUnmuted, refresh)
        .off(ParticipantEvent.LocalTrackPublished, refresh).off(ParticipantEvent.LocalTrackUnpublished, refresh)
  ...
  useEffect(() => {
    ...
    if (!el || !track || track.kind !== 'video' || track.isMuted) { setHasVideo(false); return; }
    track.attach(el); setHasVideo(true);
    return () => { track.detach(el); setHasVideo(false); };
  }, [participant, version]);
  ...
  return (
    ...
    >
      {/* ── Inner clip box — keeps video/bg/label within rounded edges ── */}
      <div className="absolute inset-0 overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[rgba(255,250,241,.7)]">
        ...
        {!hasVideo && (
          <div className={`absolute inset-0 flex flex-col items-center justify-center gap-3 bg-[linear-gradient(135deg,rgba(13,91,215,.12),rgba(47,99,64,.10))]`}>
            <div className={`grid place-items-center rounded-full bg-[rgba(13,91,215,.18)] font-bold text-[var(--brand-strong)] ${expanded ? 'h-20 w-20 text-[2rem]' : 'h-12 w-12 text-[1.15rem]'}`}>
              {initialsFromName(fallbackName)}
            </div>
            {participant.isCameraEnabled && (
              <div className={`font-medium text-[rgba(13,91,215,.8)] ${expanded ? 'text-[0.8rem]' : 'text-[0.7rem]'}`}>Camera starting…</div>
            )}
          </div>
        )}
        {/* Label gradient */}
        <div className={`absolute inset-x-0 bottom-0 flex items-center justify-between gap-2 px-3 text-white ${expanded ? 'pb-3 pt-6' : 'py-2'}`}
          style={{ background: 'linear-gradient(to top, rgba(26,26,26,.75), transparent)' }}>
          <span className={expanded ? 'text-[0.82rem] font-medium' : 'truncate text-[0.72rem]'}>
      ...
      {/* ── Floating elements — outside clip box ── */}
      ...
      {onPinToggle && (
        <button
          ...
          style={{ opacity: (hovered || isPinned) ? 1 : 0, transform: (hovered || isPinned) ? 'scale(1)' : 'scale(0.8)', transition: 'opacity 140ms, transform 140ms, background 150ms' }}
        >
          <IconPin filled={isPinned} />
        </button>
      )}

      {/* Reaction bubbles — float up freely above tile */}
      {reactions.map((r) => (
        <span key={r.id} className="media-reaction-bubble">{r.emoji}</span>
      ))}
    </div>
  );
}
...
function ScreenShareTile({ participant, identity, isLocal }: TileParticipant) {
  ...
  useEffect(() => {
    ...
    const track = pub?.videoTrack ?? pub?.track;
    if (!el || !track || track.kind !== 'video') { setHasVideo(false); return; }
    track.attach(el); setHasVideo(true);
    return () => { track.detach(el); setHasVideo(false); };
  }, [participant, version]);
  ...
  return (
    <div style={{ display: hasVideo ? 'block' : 'none' }}
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-center gap-2 bg-[rgba(13,91,215,.82)] px-3 py-1.5 text-[0.72rem] text-white">
        <IconScreen /><span className="font-medium">{label}</span>
        {isLocal && <span className="ml-auto rounded-full bg-[rgba(255,255,255,.2)] px-2 py-0.5 text-[0.65rem]">Visible to everyone</span>}
      </div>
    </div>
  );
}
    ...
    <div className="relative flex h-full w-full items-center justify-center overflow-hidden rounded-2xl border border-[rgba(13,91,215,.28)] bg-[rgba(255,250,241,.7)]">
      ...
      {!hasVideo && (
        <div className="flex flex-col items-center gap-3 text-[var(--ink-soft)]">
          <IconScreen /><div className="text-[0.9rem]">Loading shared screen…</div>
        </div>
      )}
      <div className="absolute left-3 top-3 flex items-center gap-1.5 rounded-full border border-[rgba(13,91,215,.35)] bg-[rgba(13,91,215,.12)] px-3 py-1 text-[0.72rem] font-semibold text-[#0b3ea5] backdrop-blur-sm">
        <IconScreen />{label}
        {isLocal && <span className="ml-1 opacity-60">· visible to everyone</span>}
      </div>
    </div>
  );
}
...
function CameraThumbnailTile({
  ...
  isSpeaking, isHandRaised, reactions = [], isPinned, onPinToggle,
}: TileParticipant & { isSpeaking?: boolean; isHandRaised?: boolean; reactions?: ReactionEvent[]; isPinned?: boolean; onPinToggle?: (id: string) => void }) {
  const videoRef = useRef<HTMLVideoElement | null>(null);
  const [version, setVersion] = useState(0);
  const [hasVideo, setHasVideo] = useState(false);
  const [hovered, setHovered] = useState(false);
    ...
    >
      <div className="absolute inset-0 overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[rgba(255,250,241,.8)]">
        ...
        {!hasVideo && (
          <div className="absolute inset-0 flex items-center justify-center bg-[linear-gradient(135deg,rgba(13,91,215,.12),rgba(47,99,64,.10))]">
            <div className="text-[0.95rem] font-bold text-[var(--brand-strong)]">{initialsFromName(fallbackName)}</div>
          </div>
        )}
        <div className="absolute inset-x-0 bottom-0 flex items-center justify-between gap-1 px-2 pb-1.5 pt-4 text-[0.62rem] text-white"
          style={{ background: 'linear-gradient(to top, rgba(26,26,26,.72), transparent)' }}>
          <span className="truncate">{isLocal ? `${fallbackName} (You)` : fallbackName}</span>
          {!participant.isMicrophoneEnabled && (
            <span className="shrink-0 rounded-full bg-[rgba(220,50,50,.7)] px-1 py-0.5 text-[0.56rem]">Muted</span>
          )}
        </div>
      </div>
      {isHandRaised && (
      ...
      {onPinToggle && (
        <button
          ...
          style={{ opacity: (hovered || isPinned) ? 1 : 0, transform: (hovered || isPinned) ? 'scale(1)' : 'scale(0.8)', transition: 'opacity 140ms, transform 140ms' }}
        >
          <IconPin filled={isPinned} />
        </button>
      )}
      {reactions.map((r) => (
        <span key={r.id} className="media-reaction-bubble" style={{ fontSize: '1.4rem', bottom: '28px' }}>{r.emoji}</span>
      ))}
    </div>
  );
}
...
function ParticipantsBar({ tiles, raisedHands, activeSpeakers, dark }: {
        ...
        {tiles.map((tile) => {
          ...
          return (
            <div key={tile.identity}
              ...
              <div className={`grid h-6 w-6 shrink-0 place-items-center rounded-full text-[0.62rem] font-bold ${isSpeaking ? 'bg-[rgba(34,197,94,.2)] text-[#14532d]' : dark ? 'bg-[rgba(255,255,255,.12)] text-white' : 'bg-[rgba(13,91,215,.18)] text-[var(--brand-strong)]'}`}>
                {initialsFromName(name)}
              </div>
              <span className="min-w-0 flex-1 truncate">{tile.isLocal ? `${name} (You)` : name}</span>
              <div className="flex shrink-0 items-center gap-1.5">
                {isHandUp   && <span title="Hand raised">✋</span>}
                {isSpeaking && <span className="h-2 w-2 rounded-full bg-[#22c55e]" title="Speaking" />}
                <IconMic    active={tile.participant.isMicrophoneEnabled} />
                <IconCamera active={tile.participant.isCameraEnabled} />
                {isSharing  && <span className="rounded-full bg-[rgba(13,91,215,.3)] px-1.5 py-0.5 text-[0.6rem] font-semibold text-[#7eb3ff]">Screen</span>}
              </div>
            </div>
          );
        })}
...
function RaisedHandBanner({ tiles, raisedHands }: { tiles: TileParticipant[]; raisedHands: Set<string> }) {
  const raised = tiles.filter((t) => raisedHands.has(t.identity) && !t.isLocal);
  if (!raised.length) return null;
  const names = raised.slice(0, 2).map((t) => t.participant.name || `User ${t.identity.slice(0, 6)}`);
  ...
  return (
    <div className="mx-2 mt-1 flex items-center gap-2 rounded-xl border border-[rgba(234,179,8,.35)] bg-[rgba(234,179,8,.12)] px-3 py-2 text-[0.78rem] text-[#92400e]">
      ...
      <span className="font-medium">{names.join(', ')}{extra} raised their hand</span>
    </div>
  );
}
...
function ReactionTray({ onReact, dark }: { onReact: (emoji: string) => void; dark?: boolean }) {
...
export function MediaPanel({ roomId, isExpanded = false }: MediaPanelProps) {
  ...
  const handTimeoutsRef      = useRef<Map<string, ReturnType<typeof setTimeout>>>(new Map());
  ...
  useEffect(() => {
    import('@livekit/krisp-noise-filter').then((mod) => {
  ...
  const { cameraTiles, screenTiles } = useMemo(() => {
    ...
    return { cameraTiles: all, screenTiles: all.filter((t) => t.participant.isScreenShareEnabled) };
  }, [participants, renderTick]);

  const reactionsFor = (identity: string) => reactions.filter((r) => r.identity === identity);

  // Spotlight: follows screenshare (preferred) or pinned identity
  useEffect(() => {
    if (screenTiles.length === 0) { setSpotlightSharerIdentity(null); return; }
    setSpotlightSharerIdentity((prev) =>
      prev && screenTiles.some((t) => t.identity === prev) ? prev : screenTiles[0]!.identity
    );
  }, [screenTiles]);

  // Clear pin if participant leaves
  useEffect(() => {
    if (pinnedIdentity && !cameraTiles.some((t) => t.identity === pinnedIdentity)) {
  ...
  useEffect(() => {
    const t = setInterval(() => pruneReactions(Date.now() - REACTION_TTL_MS), 500);
  ...
  const readAutoJoin  = () => typeof window !== 'undefined' && localStorage.getItem(`${MEDIA_AUTOREJOIN_PREFIX}${roomId}`) === 'true';
  const writeAutoJoin = (v: boolean) => { if (typeof window !== 'undefined') localStorage.setItem(`${MEDIA_AUTOREJOIN_PREFIX}${roomId}`, String(v)); };
  ...
  const bindRoom = useCallback((room: Room) => {
    ...
    room
      ...
      .on(RoomEvent.ParticipantConnected, (p: Participant) => { bindParticipant(p); refresh(); })
      .on(RoomEvent.ParticipantDisconnected, (p: Participant) => { setRaisedHand(p.identity, false); refresh(); })
      .on(RoomEvent.TrackSubscribed, refresh).on(RoomEvent.TrackUnsubscribed, refresh)
      ...
      .on(RoomEvent.DataReceived, (payload: Uint8Array, participant?: Participant) => {
        ...
        if (msg.type === 'raise-hand') {
          ...
          if (msg.raised) {
            const t = setTimeout(() => setRaisedHand(identity, false), RAISE_HAND_TIMEOUT_MS);
        ...
        if (msg.type === 'reaction') {
          addReaction({ id: nanoid6(), identity: participant.identity, emoji: msg.emoji, at: Date.now() });
        }
      })
  ...
  }, [syncRoomState, setStatus, setDevices, setParticipants, setActiveSpeakers, setRaisedHand, addReaction, setError]);

  // ── Actions ────────────────────────────────────────────────────────────────

  const joinMedia = async () => {
    writeAutoJoin(true); setError(null); setStatus('joining');
    const existing = roomRef.current;
    if (existing) { await existing.disconnect(true); roomRef.current = null; }
    try {
      ...
      await room.startAudio(); await room.startVideo();
      await room.localParticipant.setMicrophoneEnabled(true);
      await room.localParticipant.setCameraEnabled(true);
      syncRoomState(room); setError(null);
    } catch (err) {
      setStatus('failed');
  ...
  const leaveMedia = async () => {
    ...
    reset(); setLocalHandRaised(false); setPinnedIdentity(null);
    if (room) await room.disconnect(true);
  };

  const toggleMicrophone = async () => {
    const room = roomRef.current; if (!room) return;
    try { await room.localParticipant.setMicrophoneEnabled(!room.localParticipant.isMicrophoneEnabled); syncRoomState(room); }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed to toggle microphone.'); }
  };

  const toggleCamera = async () => {
    const room = roomRef.current; if (!room) return;
    try { await room.localParticipant.setCameraEnabled(!room.localParticipant.isCameraEnabled); syncRoomState(room); }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed to toggle camera.'); }
  };
  ...
  const toggleNoiseSuppression = async () => {
    const room = roomRef.current; if (!room || !KrispFilter) return;
    const next = !noiseSuppression;
    try {
      if (next) {
        await room.localParticipant.setMicrophoneEnabled(true, undefined, { processor: KrispFilter() } as any);
      } else {
        await room.localParticipant.setMicrophoneEnabled(room.localParticipant.isMicrophoneEnabled);
      }
      setNoiseSuppression(next); syncRoomState(room);
    } catch (err) { setError(err instanceof Error ? err.message : 'Noise suppression failed.'); }
  };
  ...
  const toggleRaiseHand = () => {
    ...
    if (next) {
      const t = setTimeout(() => {
        setLocalHandRaised(false); setRaisedHand(room.localParticipant.identity, false);
  ...
  const sendReaction = (emoji: string) => {
    ...
    // Also show locally
    addReaction({ id: nanoid6(), identity: room.localParticipant.identity, emoji, at: Date.now() });
  };
  ...
  const togglePiP = async () => {
    ...
    const el = pipVideoRef.current; if (!el) return;
    try {
      if (document.pictureInPictureElement) {
        await document.exitPictureInPicture(); setIsPiP(false);
      } else {
        await el.requestPictureInPicture(); setIsPiP(true);
  ...
  const spotlightTile = useMemo(() =>
    screenTiles.find((t) => t.identity === spotlightSharerIdentity) ?? screenTiles[0] ?? null,
    [screenTiles, spotlightSharerIdentity],
  );

  // In expanded mode: show spotlight layout if screenshare OR pinned (no screenshare)
  const pinnedTile      = pinnedIdentity ? cameraTiles.find((t) => t.identity === pinnedIdentity) ?? null : null;
      ...
      {!isInSession ? (
        ...
        <>
          ...
          <button type="button" onClick={() => void toggleScreenShare()}
            ...
            <IconScreen />{screenShareEnabled ? 'Stop sharing' : 'Share screen'}
          </button>
          <button type="button" onClick={toggleRaiseHand}
            ...
            <IconHand />{localHandRaised ? 'Lower hand' : 'Raise hand'}
          </button>
          <ReactionTray onReact={sendReaction} dark={dark} />
          {krispAvail && (
            <button type="button" onClick={() => void toggleNoiseSuppression()}
              ...
              <IconNoise />{noiseSuppression ? 'Noise ✓' : 'Denoise'}
            </button>
          )}
          {pipSupported && status === 'connected' && (
            <button type="button" onClick={() => void togglePiP()}
              ...
              <IconPiP />{isPiP ? 'Exit PiP' : 'PiP'}
            </button>
          )}
          <button type="button" onClick={() => void leaveMedia()}
  ...
  if (isExpanded) {
    return (
      <section className="glass flex h-full flex-col overflow-hidden rounded-2xl">
        ...
        <video ref={pipVideoRef} autoPlay playsInline muted style={{ position: 'absolute', width: 0, height: 0, opacity: 0, pointerEvents: 'none' }} />
        ...
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[var(--border-subtle)] px-5 py-3">
          <div>
            ...
            <div className="text-[0.76rem] soft-copy">
              {isInSession ? `${participants.length} participant${participants.length === 1 ? '' : 's'}${screenShareEnabled ? ' · Sharing screen' : ''}${pinnedIdentity ? ' · Pinned' : ''}` : 'Join voice and video for the workspace'}
            </div>
          </div>
          <div className="flex items-center gap-2">
            {pinnedIdentity && (
              <button type="button" onClick={() => setPinnedIdentity(null)}
            ...
            <span className="rounded-full border border-[rgba(13,91,215,.28)] bg-[rgba(13,91,215,.1)] px-2.5 py-1 text-[0.72rem] text-[#13489a]">{status}</span>
          </div>
        </div>
        ...
        {anyScreen ? (
          <>
            ...
            {screenTiles.length > 1 && (
              <div className="flex shrink-0 gap-1 overflow-x-auto border-b border-[var(--border-subtle)] px-4 pt-2 pb-0" style={{ scrollbarWidth: 'none' }}>
                {screenTiles.map((tile) => {
                  const name   = tile.participant.name || `User ${tile.identity.slice(0, 6)}`;
                  const active = tile.identity === spotlightSharerIdentity;
                  return (
                    <button key={tile.identity} type="button" onClick={() => setSpotlightSharerIdentity(tile.identity)}
                      ...
                      <IconScreen />{tile.isLocal ? 'Your screen' : `${name}'s screen`}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="relative min-h-0 flex-1 overflow-hidden p-4 pb-2">
              {spotlightTile && <ScreenShareSpotlightTile key={`spot-${spotlightTile.identity}`} {...spotlightTile} />}
            </div>
            {/* Camera thumbnail strip */}
            {cameraTiles.length > 0 && (
              <div className="flex shrink-0 gap-2 overflow-x-auto px-4 pb-3" style={{ height: '120px', scrollbarWidth: 'thin' }}>
          ...
          <>
            <div className="relative min-h-0 flex-1 overflow-hidden p-4 pb-2">
              <ParticipantTile key={`pin-spot-${pinnedTile!.identity}`} {...pinnedTile!}
                expanded isSpeaking={activeSpeakers.includes(pinnedTile!.identity)}
                isHandRaised={raisedHands.has(pinnedTile!.identity)} isPinned
                reactions={reactionsFor(pinnedTile!.identity)} onPinToggle={togglePin}
              />
            </div>
            {cameraTiles.length > 1 && (
              <div className="flex shrink-0 gap-2 overflow-x-auto px-4 pb-3" style={{ height: '120px', scrollbarWidth: 'thin' }}>
                {cameraTiles.filter((t) => t.identity !== pinnedIdentity).map((tile) => (
                  <CameraThumbnailTile key={`thumb-${tile.identity}`} {...tile}
                    isSpeaking={activeSpeakers.includes(tile.identity)}
                    isHandRaised={raisedHands.has(tile.identity)}
                    isPinned={false}
          ...
          <div className="relative min-h-0 flex-1 overflow-hidden p-4">
            {cameraTiles.length > 0 ? (
              <div className="grid h-full gap-3" style={{ gridTemplateColumns: gridCols(cameraTiles.length) }}>
                {cameraTiles.map((tile) => (
              ...
              <div className="flex h-full flex-col items-center justify-center gap-3 text-[var(--ink-soft)]">
                <svg width="48" height="48" viewBox="0 0 20 20" fill="currentColor" className="opacity-20">
                  <path d="M2 6a2 2 0 012-2h8a2 2 0 012 2v2l4-2v8l-4-2v2a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" />
                </svg>
                <div className="text-[0.9rem]">{isInSession ? 'No video participants yet.' : 'Join to see participants.'}</div>
              </div>
            )}
          </div>
        )}
        ...
        {isInSession && cameraTiles.length > 0 && (
          <div className="shrink-0 border-t border-[var(--border-subtle)] px-5 py-2">
            <ParticipantsBar tiles={cameraTiles} raisedHands={raisedHands} activeSpeakers={activeSpeakers} />
          </div>
        )}

        {/* Controls */}
        <div className="shrink-0 border-t border-[var(--border-subtle)] px-5 py-3">
          <div className="flex flex-wrap items-center justify-center gap-2"><ControlsBar /></div>
          {error && (
            <div className="mt-2 rounded-xl border border-[rgba(172,56,48,.24)] bg-[rgba(172,56,48,.12)] px-3 py-2 text-[0.78rem] text-[#8c2317]">{error}</div>
          )}
        </div>
      </section>
    );
    ...
    <section className="glass flex h-full min-h-0 flex-col gap-0 overflow-hidden rounded-[20px]">
      ...
      <div className="panel-header">
        ...
        <div className="flex items-center gap-1.5">
          {status === 'connected' && (
            <>
              {videoEnabled  && <span className="status-pill status-pill-blue">Camera</span>}
              {audioEnabled  && <span className="status-pill status-pill-green">Mic</span>}
              {localHandRaised && <span className="status-pill" style={{ background: 'rgba(234,179,8,.12)', borderColor: 'rgba(234,179,8,.35)', color: '#92400e' }}>✋</span>}
              {pinnedIdentity  && (
                <button type="button" onClick={() => setPinnedIdentity(null)}
                  className="status-pill" style={{ background: 'rgba(13,91,215,.1)', borderColor: 'rgba(13,91,215,.28)', color: '#13489a', cursor: 'pointer' }}>
                  📌 Unpin
                </button>
              )}
            </>
          )}
          <span className={`status-pill ${status === 'connected' ? 'status-pill-green' : status === 'joining' || status === 'reconnecting' ? 'status-pill-blue' : 'status-pill-red'}`}>
            {status === 'connected' ? 'Live' : status === 'joining' ? 'Joining…' : status === 'reconnecting' ? 'Reconnecting…' : 'Idle'}
          </span>
        </div>
      </div>
      ...
      <div className="scroll-thin flex min-h-0 flex-1 flex-col gap-2.5 overflow-y-auto p-2.5">
            ...
            {screenTiles.map((tile) => {
              ...
              return (
                <button key={tile.identity} type="button" onClick={() => setSpotlightSharerIdentity(tile.identity)}
                  ...
                  <IconScreen />{tile.isLocal ? 'You' : name}
                </button>
              );
            })}
          </div>
        ...
        {spotlightTile && <ScreenShareTile key={`screen-${spotlightTile.identity}`} {...spotlightTile} />}

        {/* Camera grid */}
        <div className={`grid gap-2 ${anyScreen ? 'grid-cols-3' : cameraTiles.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
          {cameraTiles.length > 0 ? (
            // Pinned tile first
            [...cameraTiles].sort((a) => a.identity === pinnedIdentity ? -1 : 0).map((tile) => (
            ...
            <div className="col-span-2 flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-[rgba(26,26,26,.18)] bg-[rgba(255,250,241,.56)] py-8 text-[var(--ink-soft)]">
              ...
              <span className="text-[0.8rem]">{isInSession ? 'No video participants yet.' : 'Join to see participants.'}</span>
            </div>
          )}
        </div>
        ...
        <ControlsBar />
        {error && (
          <div className="rounded-xl border border-[rgba(172,56,48,.24)] bg-[rgba(172,56,48,.12)] px-3 py-2 text-[0.78rem] text-[#8c2317]">{error}</div>
        )}
      </div>
...
function PipTileInner({ participant, identity, isLocal, isSpeaking, isHandRaised, reactions = [] }: {
  participant: Participant;
  identity: string;
  isLocal: boolean;
  isSpeaking?: boolean;
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: '80px', borderRadius: '10px', overflow: 'hidden', border: isSpeaking ? '2px solid rgba(34,197,94,.8)' : '2px solid transparent', boxSizing: 'border-box' }}>
      <video ref={videoRef} autoPlay playsInline muted={isLocal}
        style={{ width: '100%', height: '100%', objectFit: 'cover', display: hasVideo ? 'block' : 'none', background: '#111' }} />
      {!hasVideo && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg,rgba(13,91,215,.25),rgba(47,99,64,.18))' }}>
          <span style={{ color: 'white', fontWeight: 700, fontSize: '1.1rem' }}>{initialsFromName(name)}</span>
        </div>
      )}
      {isHandRaised && (
        <div style={{ position: 'absolute', right: 4, top: 4, fontSize: '1.1rem' }}>✋</div>
      )}
      {reactions.map((r) => (
        <span key={r.id} className="media-reaction-bubble" style={{ fontSize: '1.4rem', bottom: '28px' }}>{r.emoji}</span>
      ))}
      {/* Label */}
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top,rgba(0,0,0,.75),transparent)', padding: '16px 8px 6px', color: 'white', fontSize: '0.68rem', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {isLocal ? `${name} (You)` : name}{!participant.isMicrophoneEnabled && ' 🔇'}
      </div>
    </div>
  );
}
...
function PipScreenShareTile({ participant, identity, isLocal }: {
  participant: Participant;
  identity: string;
  isLocal: boolean;
}) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#0a0a0a' }}>
      ...
      {!hasVideo && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,.4)', fontSize: '0.8rem' }}>
          Loading screen…
        </div>
      )}
      {/* "Presenting" badge */}
      <div style={{
        ...
        background: 'rgba(13,91,215,.75)', backdropFilter: 'blur(6px)',
